...
const App: React.FC<AcademyAppProps> = ({ disableChat: propDisableChat, onNavigateToDashboard }) => {
  // Check URL parameters for widget mode (similar to PlanApp pattern)
  // Routes supported:
  //   ?mode=widget or ?chat=false - disables chat
  //   ?page=viewer - starts in Viewer mode
  //   ?page=hub - starts in Hub mode
  // Also check for academy-app-options element (for embedded widgets)
  const urlParams = new URLSearchParams(window.location.search);
  const el = document.getElementById('academy-app-options');
  
  // Determine if chat should be disabled
  const urlDisableChat = urlParams.get('chat') === 'false' || urlParams.get('mode') === 'widget';
  const dataDisableChat = el?.dataset.chat === 'false' || el?.dataset.mode === 'widget';
  const disableChat = propDisableChat ?? dataDisableChat ?? urlDisableChat;
  
  // Determine if header should be hidden (only when embedded as widget, not in full view)
  const urlHideHeader = urlParams.get('hideHeader') === 'true';
  const dataHideHeader = el?.dataset.hideHeader === 'true';
  const hideHeader = dataHideHeader ?? urlHideHeader;
  
  // Determine initial mode from URL or data attribute
  const urlMode = urlParams.get('page');
  const dataMode = el?.dataset.page;
  const modeParam = urlMode || dataMode;
  const initialMode = modeParam === 'hub' ? AppMode.Data : modeParam === 'viewer' ? AppMode.Viewer : AppMode.Viewer;
  ...
  const [isChatVisible, setIsChatVisible] = useState(!disableChat);
  ...
  const gemini = useMemo(() => new GeminiService("", bookData), [bookData]);
            ...
            <nav className="flex items-center gap-1">
              {onNavigateToDashboard && (
                <button 
                  ...
                  className="px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-[9px] md:text-xs font-black uppercase tracking-widest transition-all text-white/40 hover:text-white hover:bg-white/10"
                >
                  Dashboard
                </button>
              )}
