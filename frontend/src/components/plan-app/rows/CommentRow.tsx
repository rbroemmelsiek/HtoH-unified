...
const CommentRow: React.FC<CommentRowProps> = ({ element }) => {
  ...
  return (
    <div
      className={`entry__comment ${hasChildren ? 'imply-toggle-possible' : ''} ${permissions.myElement ? 'my-element' : ''} ${isHovered ? 'hovered' : ''} ${minHeightClass ? 'min-height-delete' : ''}`}
      onClick={toggleOpen}
      ...
      <div className="section">
        <div className="section section__main" ref={mainRef}>
          ...
          <div className="plan-app__entry__body" ref={bodyRef} onClick={(e) => e.stopPropagation()}>
            {!element.edit && (
              <span
                className={`entry__header deny-toggle ${isUnread ? 'bold' : ''}`}
                onMouseDown={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  if (!element.name) {
              ...
              >
                {element.name || <span style={{ opacity: 0.5, fontStyle: 'italic' }}>Click to add comment</span>}
              </span>
            )}
            {element.edit && (
              <input
