...
const requestPlanAi = async (mode: PlanAiMode, prompt: string): Promise<any> => {
  const response = await fetch(PLAN_AI_FUNCTION_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ mode, prompt }),
  });
  if (!response.ok) {
    throw new Error(`planAiAssist HTTP ${response.status}`);
  }
...
export const getBulkSubtasks = async (
  ...
  try {
    const payload = await requestPlanAi("bulkSubtasks", prompt);
    if (Array.isArray(payload?.tasks)) {
      return payload.tasks as GeneratedTask[];
    }
    return [];
  } catch (e) {
...
export const getTaskSuggestion = async (
  ...
  try {
    const payload = await requestPlanAi("taskSuggestion", prompt);
