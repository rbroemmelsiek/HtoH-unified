...
const KindleViewer: React.FC<KindleViewerProps> = ({ 
  ...
  isFullscreen, onToggleFullscreen, allowCopy, zoom, onZoomChange, isChatVisible, onToggleChat
}) => {
    ...
    >
      ...
      {isFullscreen && (
        <button 
          ...
          className="absolute top-6 right-6 z-[300] w-12 h-12 rounded-full bg-black/60 backdrop-blur-xl border border-white/20 text-white flex items-center justify-center shadow-2xl transition-all hover:bg-red-500 hover:scale-110 active:scale-90"
        >
      ...
      <div ref={containerRef} className={`flex-1 relative overflow-hidden flex flex-col items-center justify-center p-0 ${allowCopy ? '' : 'unselectable'}`}>
        {isDarkMode && (
        ...
        {isLocked ? (
          ...
          <div className="flex flex-col items-center w-full h-full overflow-hidden relative">
              ...
              <div style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'center center' }} className="transition-transform duration-300 ease-out z-10 flex flex-col items-center justify-center">
                {fileSource ? (
                  ...
                  >
                    ...
                    {/* Navigation Hotspots - Only visible when not in Fullscreen or via desktop keys */}
                    {!isFullscreen && (
                      <>
                        <div className="nav-hotspot left-0" onClick={handlePrev}>
                          <div className="hidden md:flex mobile-nav-btn w-12 h-12 md:w-20 md:h-20 rounded-full bg-black/60 text-white items-center justify-center shadow-2xl ml-2 md:ml-8 border border-white/10 hover:scale-110 transition-all backdrop-blur-xl">
                            <i className="fas fa-chevron-left text-sm md:text-2xl"></i>
                          </div>
                        </div>
                        <div className="nav-hotspot right right-0 justify-end" onClick={handleNext}>
                          <div className="hidden md:flex mobile-nav-btn w-12 h-12 md:w-20 md:h-20 rounded-full bg-black/60 text-white items-center justify-center shadow-2xl mr-2 md:mr-8 border border-white/10 hover:scale-110 transition-all backdrop-blur-xl">
                            <i className="fas fa-chevron-right text-sm md:text-2xl"></i>
                          </div>
                        </div>
                      </>
                    )}
                  ...
                  <div onClick={onUploadClick} className="w-[300px] h-[170px] md:w-[800px] md:h-[450px] flex flex-col items-center justify-center border-2 border-dashed border-white/20 rounded-[2rem] bg-white/5 p-6 md:p-20 text-center cursor-pointer hover:bg-white/10 transition-all shadow-3xl">
            ...
            {/* Video Action Buttons - Now restored in fullscreen mode */}
            {currentPageData && fileSource && !isLocked && (
              <div className={`flex-none mt-4 md:mt-2 flex flex-wrap justify-center gap-3 md:gap-8 z-[120] relative max-w-full px-4 mb-2 md:mb-1 ${isFullscreen ? 'opacity-0 hover:opacity-100 transition-opacity duration-300' : ''}`}>
                <button 
                  ...
                  className="bg-zinc-900/60 border border-white/20 rounded-xl md:rounded-2xl px-4 py-2 md:px-8 md:py-4 flex flex-row items-center gap-2 md:gap-4 shadow-3xl transition-all hover:bg-zinc-800/80 hover:scale-105 group backdrop-blur-md"
                >
