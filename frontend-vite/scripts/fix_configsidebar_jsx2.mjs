...
const re = /<section className="bg-white\/5 rounded-\[2rem\] border border-white\/5 p-6">\s*\n\s*<h3 className="text-\[10px\] font-black uppercase tracking-\[0\.3em\] text-\[#ff6600\] mb-6 Course Injection">[\s\S]*?<\/section>/m;

const replacement = `<section className="bg-white/5 rounded-[2rem] border border-white/5 p-6">
...
if (!re.test(s)) {
  throw new Error('Broken Course Injection block not matched; aborting');
}

s = s.replace(re, replacement);
fs.writeFileSync(p,s,'utf8');
console.log('Replaced broken Course Injection block');
